...
export default function PackageClient({ packageID }) {
  ...
  const fetchInclusions = async (pid) => {
    setLoadingInclusions(true);
    try {
      const d = await Data.fetch_inclusions(pid);
      if (d.status === 'success') setInclusions(d.inclusions);
    } catch (err) {
      // toast.error(err.message);
    } finally {
      setLoadingInclusions(false);
  ...
  const fetchExclusions = async (pid) => {
    setLoadingExclusions(true);
    try {
      const d = await Data.fetch_exclusions(pid);
      if (d.status === 'success') setExclusions(d.exclusions);
    } catch (err) {
      // toast.error(err.message);
    } finally {
      setLoadingExclusions(false);

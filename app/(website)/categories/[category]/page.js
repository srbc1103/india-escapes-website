...
export default function CategoryPage({params}) {
    ...
    const [state, setState] = useState({
        name: '', description:'', image: IMAGES.hero_bg
    })
    let bc_array = [
        { text: 'Home', url: `/` },
    ...
    useEffect(()=>{
        if(category_detail){
            let { name, images, description, id } = category_detail
            setState(s=>({...s,name,description,image:images[0] || IMAGES.hero_bg}))
            setCategoryID(id)
            document.title = name
        }
    },[category_detail])
    ...
    <div>
        ...
        {loading ? <>
            ...
            <div className="w_80_90 py-4 md:py-8 space-y-2 md:space-y-4">
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-red">{state.name}</h1>

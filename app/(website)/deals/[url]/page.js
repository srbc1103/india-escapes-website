...
export default function Deal({params}) {
    ...
    const { loading, data: deal_detail, load_detail: loadDeal } = useFetch({url, item_type: 'deal',collection_id: COLLECTIONS.DEALS})
    ...
    async function LoadDeals(deals){
        ...
        Data.fetchPackagesBundles(deals,'deals').then(d=>{
            ...
            {state.images && state.images.length > 0 ? <>
                ...
                >
                    ...
                    return <SwiperSlide key={index} className={`w-full  h-full`}>
                        <Image src={img} alt={name} height={1000} width={1000} className='w-full h-full object-cover opacity-90'/>
                    </SwiperSlide>
                    })}
                </Swiper>

...
export default function ACCOMMODATIONS() {
  ...
  const {
    ...
    clearSearch,
    handleSearch
  } = useCMSSearch({ collection_id: COLLECTIONS.ACCOMMODATIONS });
  ...
  async function get_items() {
    ...
    Data.get_items_list({ collection_id: COLLECTIONS.ACCOMMODATIONS, offset }).then(d => {
      let { status } = d;
      if (status === 'success') {
        let { total, items } = d;
  ...
  const handleDelete = async (oid) => {
    let cnf = confirm('Are you sure you want to delete this accommodation? This action cannot be undone');
    if (!cnf) return;
    setUpdating(true);
    Data.delete_item({ collection_id: COLLECTIONS.ACCOMMODATIONS, document_id: oid }).then(d => {
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await Data.get_all_items_for_export({
        collection_id: COLLECTIONS.ACCOMMODATIONS
      });

      if (response.status === 'success' && response.items.length > 0) {
        downloadCSV(response.items, 'accommodations.csv');
    ...
    <div className="w-full py-8 px-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={showLocationForm} styles="text-sm">+ New Accommodation</Button>
        </div>
      </div>

...
export default function DEALS() {
  ...
  async function get_items() {
    ...
    Data.get_items_list({ collection_id: COLLECTIONS.DEALS, offset }).then(d => {
      let { status } = d;
      if (status === 'success') {
        let { total, items } = d;
  ...
  const handleDelete = async (oid) => {
    let cnf = confirm('Are you sure you want to delete this deal? This action cannot be undone');
    if (!cnf) return;
    setUpdating(true);
    Data.delete_item({ collection_id: COLLECTIONS.DEALS, document_id: oid }).then(d => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (response.status === 'success' && response.items.length > 0) {
        downloadCSV(response.items, 'deals.csv');
    ...
    <div className="w-full py-8 px-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={showLocationForm} styles="text-sm">+ New Deal</Button>
        </div>
      </div>
          ...
          <table className="w-full max-w-4xl table-auto text-sm rounded-xl overflow-hidden shadow-lg">
            <thead>
              <tr className="bg-gray-100 text-black">
                ...
                <th className="px-4 py-3 text-left text-xs uppercase font-medium">Discount Rate</th>
                ...
                state.filtered_items.map((item, index) => (
                  <tr key={item.id} className="border-b border-gray-200/50 dark:border-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/ie_cms/deal?did=${item.id}`}
                    ...
                    <span className={`${item.active ? 'text-green-500' : ''} `}>{item.active ? 'Active' : 'Not Active'}</span>
                    {item.active ? <span className="block text-[10px] text-gray-500 whitespace-nowrap">{`Ends on ${moment(item.end_date)?.format('DD MMM')}`}</span> : <></>}
                    </td>
                    <td className="px-4 py-3 text-gray-800 dark:text-gray-200">{`${item.discount_rate}% off`}</td>
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="bg-white text-sm w-[150px] -mt-2 absolute -right-4 p-0 rounded-xl shadow-lg border-none overflow-hidden" style={{ zIndex: 100 }}>
                          <button
                            ...
                            role="button"
                            onClick={() => {
                              router.push(`/ie_cms/deal?did=${item.id}`)
                            }}
                            disabled={updating}
                          >

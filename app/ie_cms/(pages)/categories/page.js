...
export default function CATEGORIES() {
  ...
  async function get_items() {
    ...
    Data.get_items_list({ collection_id: COLLECTIONS.CATEGORIES, offset }).then(d => {
      let { status } = d;
      if (status === 'success') {
        let { total, items } = d;
  ...
  const handleDelete = async (oid) => {
    let cnf = confirm('Are you sure you want to delete this category? This action cannot be undone');
    if (!cnf) return;
    setUpdating(true);
    Data.delete_item({ collection_id: COLLECTIONS.CATEGORIES, document_id: oid }).then(d => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (response.status === 'success' && response.items.length > 0) {
        downloadCSV(response.items, 'categories.csv');
    ...
    <div className="w-full py-8 px-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={showLocationForm} styles="text-sm">+ New category</Button>
        </div>
      </div>
          ...
          <table className="w-full max-w-4xl table-auto text-sm rounded-xl overflow-hidden shadow-lg">
            <thead>
              <tr className="bg-gray-100 text-black">
                ...
                <th className="py-3 text-left text-xs uppercase font-medium">Featured Image</th>
              ...
              {loading ? (
                ...
                Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className="border-b dark:border-gray-700">
                    ...
                    <td className="px-4 py-3"><Skeleton className="h-4 w-4 rounded-full bg-gray-200 dark:bg-gray-700" /></td>
                ...
                state.filtered_items.map((item, index) => (
                  <tr key={item.id} className="border-b border-gray-200/50 dark:border-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td>
                      {item.featured_image ? <Image height={100} width={100} src={item.featured_image} alt={item.name} className="h-12 w-12 object-cover rounded-full"/> : <></>}
                    </td>
